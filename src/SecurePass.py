...
if __name__ == "__main__":
	...
	if m_choice == 1:
		...
		while MCM_instance.is_master_username_exists(PasswordChecker.calculate_md5(m_user), FireBase):
			Printer.bold()
		...
		FireBase.m_user, FireBase.authHash = PasswordChecker.calculate_md5(m_user), authHash	
		...
		if vault is not None:
			...
			decryptedVault = EncryptionUtil.decryption(vault)
			Printer.reset()
		else:
			Printer.bold()
	...
	while True:
		...
		if choice == 1:
			...
			decryptedVault = password_manager.add_password(authHash, decryptedVault, FireBase, EncryptionUtil)
			...
			if not password_manager.displayAll(decryptedVault):
				...
				if sno != "":
					recoveredVault = password_manager.removePassword(int(sno), decryptedVault, FireBase, EncryptionUtil)
