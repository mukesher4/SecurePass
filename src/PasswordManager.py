...
class PasswordManager:
	...
	def add_password(self, authHash, m_key, decryptedVault, FireBase, EncryptionUtil):
		print("\t\t\t\t\t\t(1) Website")
		print("\t\t\t\t\t\t(2) Application")
		print("\t\t\t\t\t\t(3) E-Mail")
		print("\t\t\t\t\t\t(4) Return back")
		...
		if account_type_choice==4:
			...
			FireBase.insertData(EncryptionUtil.encryption(decryptedVault, m_key))

			return decryptedVault

	def removePassword(self, sno, decryptedVault, FireBase, EncryptionUtil, m_key):
		tempData = decryptedVault.split()
		if len(tempData) >= sno:
			del tempData[sno-1]
			decryptedVault = '\n'.join(tempData)
			FireBase.insertData(EncryptionUtil.encryption(decryptedVault, m_key))
	...
	def displayAll(self, decryptedVault):
		number_of_records=0
		record=[]
		for line in decryptedVault.split()[:-1]:
			data = line.split("~^~")
			number_of_records+=1
			password = data[-1]
